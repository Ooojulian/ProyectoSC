...
MAX_NODES = 4  # nodos cliente máximos (Node-2 a Node-5)

def _next_assignment(hello: dict) -> dict:
    with _lock:
        idx  = len(_assigned)
        if idx >= MAX_NODES:
            LOG.warning(f"Máximo de nodos ({MAX_NODES}) alcanzado, reasignando slot {idx % MAX_NODES + 1}")
        role   = ROLE_SEQUENCE[idx % len(ROLE_SEQUENCE)]
        num    = (idx % MAX_NODES) + 2   # siempre dentro de Node-2..Node-5
        name   = f"Node-{num}"
        ...
        LOG.info(f"Asignado {name} rol={role} bat0_ip={bat_ip}")
        return record
