#!/bin/bash
# =============================================================================
# ProyectoSC — Red Mesh P2P con BATMAN-adv  [Fedora]
# Uso: sudo bash startfedora.sh
#
# Todos los nodos son iguales. No hay master ni cliente.
# Cada nodo:
#   1. Instala dependencias del sistema (dnf)
...
# ─── 0. Verificar root ───────────────────────────────────────────────────────
if [ "$EUID" -ne 0 ]; then
    error "Ejecutar con sudo: sudo bash startfedora.sh"
fi

echo ""
echo "============================================================"
echo "  ProyectoSC — Nodo P2P Mesh  [Fedora]"
echo "============================================================"
echo ""
...
info "Instalando dependencias del sistema (dnf)..."

# En Fedora, batman-adv viene en el paquete del kernel extra o kmod-batman-adv
# via RPM Fusion. Intentamos habilitar RPM Fusion si no está activo.
if ! rpm -q rpmfusion-free-release &>/dev/null; then
    info "Habilitando RPM Fusion (necesario para kmod-batman-adv)..."
    FEDORA_VER=$(rpm -E %fedora)
    dnf install -y -q \
        "https://mirrors.rpmfusion.org/free/fedora/rpmfusion-free-release-${FEDORA_VER}.noarch.rpm" \
        2>/dev/null || warn "No se pudo habilitar RPM Fusion — batman-adv puede fallar"
fi
...
dnf install -y -q python3-flask 2>/dev/null || \
...
# En Fedora, NetworkManager puede resistirse más; usamos nmcli para desconectar
# la interfaz WiFi sin detener todo NM (útil si hay ethernet activo).
info "Desconectando interfaz WiFi de NetworkManager..."
nmcli device disconnect "$WIFI_IF" 2>/dev/null || true
nmcli device set "$WIFI_IF" managed no 2>/dev/null || true
sleep 2
...
# ─── 5. Asignación de IP fija para nodo Fedora ───────────────────────────────
# IP reservada para el nodo Fedora del equipo (juan).
BATMAN_IP="${NET_PREFIX}.106"

ip addr flush dev bat0 2>/dev/null || true
ip addr add "$BATMAN_IP/24" dev bat0
ok "IP asignada: $BATMAN_IP (nodo Fedora fijo)"

# ─── 6. Abrir firewall para el dashboard ─────────────────────────────────────
# Fedora usa firewalld por defecto; abrimos el puerto 5080 temporalmente.
if systemctl is-active --quiet firewalld 2>/dev/null; then
    info "Abriendo puerto 5080 en firewalld..."
    firewall-cmd --add-port=5080/tcp --timeout=3600 2>/dev/null || \
        warn "No se pudo abrir puerto en firewalld — dashboard puede no ser accesible desde otros nodos"
fi

# ─── 7. Guardar configuración del nodo ───────────────────────────────────────
